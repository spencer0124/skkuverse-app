...
- 빌드 스크립트에서 `eas build:version:get` → +1 → `eas build:version:set`으로 수동 증가
- `app.config.ts`의 `buildNumber`는 무시됨 (expo-constants manifest에만 남음)

## Release Notes (변경사항)

배포 시 App Store / TestFlight에 표시되는 변경사항. `fastlane/metadata/ios/` 아래 locale별 디렉토리에 관리.
...
**사용법:** 배포 전에 `release_notes.txt` 파일을 수정하면 업로드 시 자동 포함됨. 수정 안 하면 기존 내용 그대로 올라감.
