...
- **Map chips are server-driven.** `GET /map/config` serves `chips` beside `layers`; a chip carries an **action** (`webview` or `focus`) and a layer set. The rule most easily got wrong: a `focus` chip's `layerIds` mean **"within this `chipGroupId`, set exactly these"** — named on, unnamed sibling off, everything outside the group untouched — so the write has to resolve the group first. That resolution lives once, in `packages/shared/src/map/chips.ts`, and no call site repeats it. Which chip is active is derived from layer visibility, never stored — and that derived answer is also what swaps the event list into the campus sheet in place of the feed. The list's rows are `selectVisibleItems` (`packages/shared/src/eventmap/derive.ts`) over the same `isLayerVisible` the render loop uses, so a row is listed exactly when its layer is drawn (a pin additionally observes its own time window; the row shows that as a status badge). Contract: `docs/reference/map-config-api-spec.md`; rendering: `docs/explanation/eventmap-rendering.md`.
