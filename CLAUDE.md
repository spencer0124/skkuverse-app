...
**iOS 26 NativeTabs minimize-on-scroll 룰 (chain root rule):** `<NativeTabs minimizeBehavior="onScrollDown">` 발화 조건은 각 탭 화면의 RNSScreen `subviews[0]` 직계가 `ScrollView`/`SectionList`/`FlatList`여야 한다는 것. **outer wrapping `<View>` 한 겹만 추가돼도 minimize 영구 비활성**. native finder(`RNSScrollViewFinder.mm`)가 strict subviews[0] chain을 명목상 끝까지 따라가지만, `mountChildComponentView(index==0)` 호출 타이밍이 첫 자식의 자식들이 mount되기 전이라 사실상 **1단계 깊이만 보장**되기 때문. 화면 root는 ScrollView/SectionList를 직계 반환하거나 Fragment(`<>...</>`)의 첫 자식으로 둘 것. selector/header overlay는 absolute positioning + Fragment 두 번째 이후 자식으로 (`<><SectionList listHeaderHeight=…/><View style={absoluteOverlay}>…</View></>` 패턴). loading/empty/error는 SectionList의 `ListEmptyComponent` + `contentContainerStyle.flexGrow:1`로 흡수. 자세한 패턴/안티패턴/native 메커니즘: `docs/ios-26-native-tabs-minimize.md`.
