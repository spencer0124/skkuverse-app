...
- **The festival is gated client-side, and `chipGroupId` is the discriminator.** A layer with a non-null `chipGroupId` is festival content, so `withoutFestival` (`packages/shared/src/map/festival.ts`) strips those layers and every chip. That one edit closes everything, because the booths' endpoint is read off those same layers: no festival layer means no `/map/markers/event` request, hence no pins, no list, no peek sheet and no `?place=` deep link. Applied once, in `CampusScreen`, right after `useMapConfig`; what decides is `isFestivalUnlocked()` (`apps/mobile/src/features/map/festivalGate.ts`), whose `FESTIVAL_OVERRIDE` is the one line that changes on festival day — and the same switch empties the campus sheet's SDUI feed. Detail: `docs/explanation/eventmap-rendering.md` §9.
- **The event map has no endpoint of its own.** The `/eventmap/manifest` + snapshot tier was deleted server-side (skkuverse-server `684e10e`); a place is one document carrying `subtitle`, `hours[]`, `fields[]` and `actions[]` on the ordinary marker wire, so the pins the map already fetches ARE what the list and the peek sheet render. Openness is `hours.length === 0 || hours.some(...)` — **empty means always open** — and one pin per coordinate is `resolvePinCollisions` (open now → `pinPriority` → next opening → `order`, `id`), which must never be run over the building layers. Contracts: skkuverse-server `docs/reference/{map-markers-api,event-places}.md`.
