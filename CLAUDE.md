...
- **Push notifications:** FCM via `@react-native-firebase/messaging` + `@notifee/react-native`. **Phase 1** (토큰 수신·딥링크) + **Phase 2** (Firestore `users`/`preferences`/`devices` bootstrap) + **Phase 3** (설정 UI + 뱃지 + foreground display) 완료. 옵션 D — 알림함 없음, 뱃지는 Zustand+Notifee 로컬. 자세한 내용은 `docs/plans/fcm-push-notifications.md`. 임시 진단 화면 `app/debug-fcm.tsx` — 캠퍼스 탭 우상단 빨간 "FCM" 버튼으로 진입 (dogfooding 안정 후 제거). 2026-04-23 세션의 `[fcm-diag]` 진단 로그는 디버깅 종료 후 전부 제거되어 원래 dev-only warn 패턴으로 복원됨 — 재진단 필요 시 기법은 아래 "Firestore 디버깅 기법" bullet + `docs/plans/fcm-push-notifications.md` Phase 3 디버깅 기법 정리 섹션 참조.
- **FCM preferences ↔ devices drift (2026-04-23 해결):** Cloud Function `syncPreferencesToDevices` (2nd gen, `asia-northeast3`, Node 22) 배포 완료. `users/{uid}/preferences/main` onWrite trigger → 해당 uid 의 active devices 모두에 `subscribedTopics` + `notificationsEnabled` **두 필드만** whitelist update (token/locale/appVersion/lastActive 등 per-device 필드 절대 안 건드림 — 다기기 안전). 실측 latency ~0.3초. 구현: `functions/src/sync-preferences-to-devices.ts`. 설정: `retry: true` + 10분 event age guard + `maxInstances: 10` + before/after diff Set 비교 + `firebase-admin/firestore` admin SDK (rules 우회). 배포/운영 주의사항 (retry amplification, Admin SDK rules bypass 코드 리뷰 포인트, budget alert 권장) 는 `docs/plans/fcm-push-notifications.md` Phase 3 디버깅 기록 #2 섹션 참조.
- **FCM delivery path (Phase 4, 2026-04-23 배포 완료):** `sendNotification` HTTP CF (2nd gen, asia-northeast3, Node 22) + `handleNoticeNotification` internal handler. Endpoint: `https://asia-northeast3-skkubus-95723.cloudfunctions.net/sendNotification`. 인증: `X-API-Key` 헤더 vs Secret Manager `FCM_API_KEY` — `defineSecret` 바인딩 + `timingSafeEqual` + **`.trim()` 방어** (openssl pipe 저장 시 trailing `\n` 포함되어 모든 valid 호출이 401 되던 함정; 저장 방식 안 타는 방어). devices 쿼리는 composite index 필수 — `apps/mobile/firestore.indexes.json` (active + notificationsEnabled + subscribedTopics). **Critical cleanup 정책: `TOKEN_CLEANUP_CODES` allowlist 에 `registration-token-not-registered` + `invalid-registration-token` 두 개만 포함** — `messaging/invalid-argument` 는 의도적 제외 (payload-wide 에러라 포함 시 bad payload 한 건으로 healthy 500 기기가 `active:false` 로 꺼지는 데이터 손실 footgun). 5KB body 로 invalid-argument 유도 → `cleanedUp:0` 회귀 테스트 통과 (E2E 검증 완료). FCM `data` payload 는 `Record<string, string>` 으로 빌드 (optional 필드 undefined 는 제외 — v1 API validation 런타임 throw). 구조화 로깅: `logger.info('notice.dispatch.complete', { noticeId, topics, deviceCount, sent, failed, cleanedUp, durationMs })` → Cloud Logging 에서 `jsonPayload.noticeId="..."` 필터로 추적. 구현: `functions/src/{send-notification,handle-notice,channels,types}.ts`. 백엔드 연동은 별도 레포 — payload 계약 (`NoticeNotificationPayload`) 공유 필요.
- **Firestore 디버깅 기법 (2026-04-23 확립):** (1) `onSnapshot` 에 `{ includeMetadataChanges: true }` 없이는 서버 ack / fromCache 전환이 emit 되지 않아 "write 미도달" 오진 유발 — 디버깅 시 반드시 켤 것. (2) Firebase Console 은 자체 listener 사용하는 웹앱이라 stale 뷰 보일 수 있음 → server truth 는 Firestore REST API (`firestore.googleapis.com/v1/projects/.../documents/<path>`) 로 Firebase CLI refresh_token 활용해 직접 조회. `~/.config/configstore/firebase-tools.json` 에 저장됨. (3) 복제 필드의 drift 검증은 두 replica 의 `updateTime` 비교로 first-step 판정.
- **FCM auth transition (Task #12, 2026-04-23):** anon→Google 및 Google→anon(sign-out) 시 `devices/{deviceId}.uid`가 stale되어 `firestore/permission-denied`를 발생시키던 버그 해결. `useAppInit`의 `onAuthStateChanged`가 `authStore.lastKnownUid`로 uid 전환을 감지해서 `initializeFirestoreNotifications()` 재실행. `withRetry` closure는 `getAuth().currentUser?.uid`를 매 attempt마다 lazy resolve (race-safe). `signOutFromGoogle`은 sign-out 전에 `unregisterDevice(deviceId)`로 doc을 `active: false` 처리. Firestore rule은 "active 문서는 owner만, inactive 문서는 아무 authed user가 claim 가능" 시맨틱 — `firestore.rules:38–66` SECURITY TRADE 주석 + `firestore.rules.test.mjs` 13 케이스 참조. Rules는 2026-04-23 `skkubus-95723`에 배포 완료.
