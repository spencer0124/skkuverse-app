...
- **Push notifications (FCM):** 옵션 D — 알림함 없음, 뱃지는 로컬(Zustand+Notifee), 결정 기록 `docs/decisions/0002`. **전체 아키텍처: `docs/explanation/fcm-architecture.md`** (v5 SSOT·tabsContract·drift sync·delivery·auth transition). 히스토리는 `docs/plans/fcm-push-notifications.md` (superseded), preferences↔devices drift 사고 기록은 `docs/internal/2026-04-fcm-preferences-devices-drift.md`. 진단 화면 `app/debug-fcm.tsx`는 orphan (진입 버튼 제거됨) — 제거 후보.
