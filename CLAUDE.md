...
- **Map chips are server-driven.** `GET /map/config` serves `chips` beside `layers`; a chip carries an **action** (`webview` or `focus`) and a layer set. The rule most easily got wrong: a `focus` chip's `layerIds` mean **"within this `chipGroupId`, set exactly these"** — named on, unnamed sibling off, everything outside the group untouched — so the write has to resolve the group first. That resolution lives once, in `packages/shared/src/map/chips.ts`, and no call site repeats it. Which chip is active is **stored** (`useMapLayerStore.chip`) rather than derived, and a chip carrying `isReset` means *stop narrowing* rather than "show these layers" — both reversed from the old derived form, because "restore what was there before" is not recoverable from the current state and because the default view now moves with the clock. The active chip is also what swaps the event list into the campus sheet in place of the feed. The list's rows are `selectVisibleMarkers` (`packages/shared/src/map/list.ts`) over the same `isLayerVisible` the render loop uses, so a row is listed exactly when its layer is drawn (a pin additionally observes its own time window; the row shows that as a status badge). Contract: `docs/reference/map-config-api-spec.md`; rendering: `docs/explanation/eventmap-rendering.md`.
- **A layer's default visibility is a function of the time of day.** `/map/config` serves `defaultVisibleWhen` per layer — `always`, `never`, or `scheduled` with recurring `"HH:MM"` KST windows where `start > end` wraps past midnight (주점 is `18:00`–`00:00`). <!-- conventions:allow-korean: the layer label the app shows --> Resolution is four tiers, `forced ?? chipNarrowing ?? userToggle ?? defaultVisibleAt(layer, now)`, and every tier is a **fallback rather than an assignment**: `useMapLayerStore` holds only what the user expressed, because writing a resolved value down freezes the schedule and makes a user's choice indistinguishable from the server's. Two rules worth keeping in mind: the KST minute is derived from the **epoch** (`packages/shared/src/map/daily-window.ts`), never `Date.getHours()`, so a wrongly-zoned phone still switches correctly; and an **unreadable declaration is OFF**, since this axis exists to put less on screen — with the layer still toggleable, and a whole-config fallback to `DEFAULT_MAP_CONFIG` if nothing is readable. The layers' windows must stay in `CampusScreen`'s `useWindowClock` input: a hidden layer is unmounted and arms no timer of its own.
- **The festival is gated client-side, and `chipGroupId` is the discriminator.** A layer with a non-null `chipGroupId` is festival content, so `withoutFestival` (`packages/shared/src/map/festival.ts`) strips those layers and every chip. That one edit closes everything, because the booths' endpoint is read off those same layers: no festival layer means no `/map/overlays/event` request, hence no pins, no list, no peek sheet and no `?place=` deep link. Applied once, in `CampusScreen`, right after `useMapConfig`; what decides is `isFestivalUnlocked()` (`apps/mobile/src/features/map/festivalGate.ts`), whose `FESTIVAL_OVERRIDE` is the one line that changes on festival day — and the same switch empties the campus sheet's SDUI feed. Detail: `docs/explanation/eventmap-rendering.md` §9.
- **The event map has no endpoint of its own.** The `/eventmap/manifest` + snapshot tier was deleted server-side (skkuverse-server `684e10e`); a place is one document carrying `subtitle`, `hours[]`, `fields[]` and `actions[]` on the ordinary overlay wire, so the overlays the map already fetches ARE what the list and the peek sheet render. Openness is `hours.length === 0 || hours.some(...)` — **empty means always open** — and one pin per coordinate is `resolvePinCollisions` (open now → `pinPriority` → next opening → `order`, `id`), which must never be run over the building layers. Contracts: skkuverse-server `docs/reference/{map-overlays-api,event-places}.md`.
- **One overlay collection per source, and an overlay names its own renderer.** `GET /map/overlays/{campus,event}` returns `data.overlays`, a `kind`-tagged union (`marker` | `polygon` | `path`) carrying RFC 7946 `geometry`. `MapLayerDef.type` is gone: a layer selects by `layerId` and each overlay picks its component, so one layer can hold pins, a zone and a route line. Three rules are load-bearing. **`kind` is an OPEN enum** — five renderers are reserved and ship without a client release, so an unknown one drops that single overlay (the `OVERLAY_KINDS` allowlist in `parser.ts` absorbs it) and **never** an exhaustive `never` assertion, which would blank a layer on an old build. **The `[lng, lat]` → `{lat, lng}` rename happens once**, in `toLatLng` (`packages/shared/src/map/geometry.ts`), because an axis swap never throws — it just draws in the Yellow Sea. And **polygon rings are reversed unconditionally** in `apps/mobile/src/features/map/utils/overlayGeometry.ts`: the wire is wound RFC 7946 (exterior CCW), Naver wants the opposite, and a wrongly wound ring still *draws* while silently refusing taps — so the verification is pressing the zone, not looking at it. Contract: `docs/reference/map-config-api-spec.md`.
- **A place marker is a dot, and only the selected one is a pin.** The axis is `style.shape` on the
  layer — `dotThenPin` (the default), `dot`, `pin` — resolved by `resolveMarkerGeometry`
  (`apps/mobile/src/features/map/utils/markerShape.ts`) and nowhere else. **Absent means
  `dotThenPin`**, so the behaviour ships with or without a server change, and the field is only an
  override. It lives on `style` rather than as a fourth `markerStyle` because the two fail in
  opposite directions: an unrecognised `markerStyle` falls through to the building-number branch
  (a new member would draw ~100 booths as green numbered circles on older builds), while an
  unrecognised `shape` reads as "the server did not say" and the client keeps its default. Three
  rules that bite: **the anchor moves with the shape** (a teardrop hangs by its tip, a disc by its
  centre, so swapping the image alone slides the marker off its coordinate); **both states stay
  images**, because Naver's tint blends rather than replaces, so one black-disc-in-a-white-ring PNG
  serves every category colour and no child View re-enters the Android bitmap race; and the dot
  asset's **transparent padding is the tap target**, so `style.size` names the visible disc and the
  overlay is told `size * DOT_CANVAS_RATIO`. Only `placeDot` reads any of this — `numberCircle`
  building markers are untouched and still take their green from `SdsColors.brand`. Detail:
  `docs/explanation/eventmap-rendering.md` §6.1.
...
- **The cold-start anonymous sign-in never blocks launch.** `anonymousSession` (`apps/mobile/src/services/anon-session.ts`) makes one attempt, lets launch move on signed out after a timeout or a failure, and retries in the background with jittered backoff. The per-IP sign-up quota counts every anonymous account, so behind a shared NAT the old blocking `await` was a dead-end error screen. Two rules: an attempt re-checks `currentUser` immediately before calling, because `signInAnonymously` **replaces** a signed-in Google user; and any Google sign-in path must `pause()` the session first and `resume()` after, as `signInWithDeviceMigration` does. A failure is logged once per process as `auth/anon-signin`.
